...
describe('buildPackage — experience.json (play pass-through)', () => {
  ...
  it('packages a normalized album as independently identified track folders and components', async () => {
    ...
    expect(config.archive.overview).toMatchObject({ title: 'Retrospect', track_count: 2, disc_count: 1 });
    expect(config.archive.tracks[1]).toMatchObject({ title: 'Bora', primary_artist: 'Guest', hidden: true });
    expect(config.history).toMatchObject({ scope: 'release_copy', edition: { applies_to: 'release' } });
    expect(config.history).not.toHaveProperty('track_ownership');
    ...
    const history = JSON.parse(await zip.file('history.json').async('string'));
    expect(archive.package_contents).toHaveLength(manifest.components.length);
    expect(archive.tracks[0].primary_audio.component_id).toBe(project.tracks[0].primary_audio_ref);
    expect(history.track_creation_history).toHaveLength(2);
