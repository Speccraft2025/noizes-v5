  ...
  <div class="relative z-10 w-full max-w-sm">
    ...
    <div class="glass rounded-2xl p-8">
      ...
      <form method="POST" action="?/google"
        use:enhance={() => { googleLoading = true; return async ({ update }) => { googleLoading = false; update(); }; }}>
        <button type="submit"
          class="w-full flex items-center justify-center gap-3 py-3 rounded-xl font-semibold text-sm mb-4 transition-all duration-200"
          style="background: rgba(255,255,255,0.06); border: 1px solid var(--border-dim); color: #fff;"
