<script>
  ...
  onMount(() => {
    ...
    // Drop the fragment so a refresh doesn't resurrect a stale error.
    ...
    <div class="glass rounded-2xl p-8">
      ...
      {:else if notice === 'auth_failed'}
        <div class="rounded-xl px-4 py-3 mb-5 text-sm"
          ...
          <span style="color: var(--ink-muted);">If you were invited, the link may have expired or already been used — ask us for a fresh one.
            Otherwise, request access on the <a href="/" class="text-white hover:underline">homepage</a>.</span>
        </div>
      {/if}
